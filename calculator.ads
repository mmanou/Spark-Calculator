...
is
    ...
    procedure Unlock (C : in out Calculator; P : in PIN.PIN) with
       Pre  => Is_Locked (C),
       Post =>
        PIN."=" (Get_Pin(C), Get_Pin(C'Old)) and Stack."=" (Get_Stack(C), Get_Stack(C'Old)) and
    ...
    procedure Plus (C : in out Calculator) with
       Pre  => not Is_Locked (C),
       Post =>
        PIN."=" (Get_Pin(C), Get_Pin(C'Old)) and Is_Locked(C) = Is_Locked(C'Old) and
       ...
       -- not enough inputs => do nothing
         ...
         (Get_Stack(C'Old).Length < 2 and Stack."=" (Get_Stack(C), Get_Stack(C'Old))));
    ...
    procedure Push (C : in out Calculator; I : Integer) with
       Pre  => not Is_Locked (C),
       Post => PIN."=" (Get_Pin(C), Get_Pin(C'Old)) and Is_Locked(C) = Is_Locked(C'Old);
    ...
    procedure Load (C : in out Calculator; V : VariableStore.Variable) with
       ...
       Post =>
        PIN."=" (Get_Pin(C), Get_Pin(C'Old)) and Is_Locked(C) = Is_Locked(C'Old) and
        (Stack."=" (Get_Stack(C), Get_Stack(C'Old)) or Get_Stack(C).Length - 1 = Get_Stack(C'Old).Length);
    ...
    procedure Store (C : in out Calculator; V : VariableStore.Variable) with
       ...
       -- not enough inputs => do nothing
         ...
         (Get_Stack(C'Old).Length < 1 and Stack."=" (Get_Stack(C), Get_Stack(C'Old))));
